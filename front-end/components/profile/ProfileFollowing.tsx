import classes from "./ProfileFollowing.module.css";
import UserImg from "../../public/user.png";
import { useDispatch } from "react-redux";
import { popupActions } from "../../store/index";

import Link from "next/link";
import { useEffect, useRef } from "react";

const ProfileFollowing: React.FC<{ following: []; title: string }> = (
  props
) => {
  const followingOverlay = useRef<HTMLDivElement>(null);
  const dispatch = useDispatch();
  const closePopup = () => {
    dispatch(popupActions.popupState(""));
  };
  document.onclick = (e: any) => {
    if (e.target == followingOverlay.current) {
      closePopup();
    }
  };
  document.body.style.overflow = "hidden";
  return (
    <div ref={followingOverlay} className={classes["overlay"]}>
      <div className={classes["following-container"]}>
        <div className={classes["following-header"]}>
          <h2>{props.title}</h2>

          <svg
            width="25"
            aria-hidden="true"
            focusable="false"
            data-prefix="fas"
            data-icon="times"
            role="img"
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 352 512"
            onClick={closePopup}
          >
            <path
              fill="currentColor"
              d="M242.72 256l100.07-100.07c12.28-12.28 12.28-32.19 0-44.48l-22.24-22.24c-12.28-12.28-32.19-12.28-44.48 0L176 189.28 75.93 89.21c-12.28-12.28-32.19-12.28-44.48 0L9.21 111.45c-12.28 12.28-12.28 32.19 0 44.48L109.28 256 9.21 356.07c-12.28 12.28-12.28 32.19 0 44.48l22.24 22.24c12.28 12.28 32.2 12.28 44.48 0L176 322.72l100.07 100.07c12.28 12.28 32.2 12.28 44.48 0l22.24-22.24c12.28-12.28 12.28-32.19 0-44.48L242.72 256z"
            ></path>
          </svg>
        </div>

        <div className={classes["following-list"]}>
          {props.following.map((val: any) => {
            return (
              <div
                key={val._id}
                className={classes["following-list-item-container"]}
              >
                <div className={classes["following-list-item"]}>
                  <img
                    className={classes["profile-img"]}
                    src={UserImg.src}
                    alt="user default image"
                  />
                  <Link href={`/${val.username}`}>
                    <h3
                      onClick={closePopup}
                      className={classes["list-username"]}
                    >
                      @{val.username}
                    </h3>
                  </Link>
                </div>
              </div>
            );
          })}
        </div>
      </div>
    </div>
  );
};
export default ProfileFollowing;
